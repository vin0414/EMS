...
class ManageController extends BaseController
{
    ...
    public function generateRentExpense()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $builder->select('Date');
                $builder->WHERE('DATE_FORMAT(Date,"%m")',$month);
                $builder->WHERE('DATE_FORMAT(Date,"%Y")',$year);
                $data = $builder->get();
                if($row = $data->getRow())
                {
                    echo "Invalid! ".$row->Date." already generated";
                }
                else
                {
                    for($i=0;$i<$count;$i++)
    ...
    public function getAllRentExpense()
    {
        ...
        $builder->join('(Select rpID,rentalID,Balance,Payment,NewBalance,LifeSpan from tblrental_payment ORDER BY rpID DESC) b','b.rentalID=a.rentalID','INNER');
